    ...
    transactionView(0)
{
    ...
    QAction *viewOnMegchain = new QAction(tr("Show transaction on Megchain"), this);
    ...
    contextMenu->addSeparator();
    contextMenu->addAction(viewOnMegchain);
    ...
    connect(viewOnMegchain, SIGNAL(triggered()), this, SLOT(viewOnMegchain()));
}
...
void TransactionView::viewOnMegchain()
{
    QModelIndexList selection = transactionView->selectionModel()->selectedRows();
    if(!selection.isEmpty())
    {
        QString format("http://megchain.info/tx/");
